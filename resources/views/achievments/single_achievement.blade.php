@extends('layouts.app')
@section('title',$achievment['title_'.$lang])
@section('content')
    <section class="breadcrumbs-custom breadcrumbs-custom-svg" style="background: {{$style->value=='pink'?'linear-gradient(-39deg, #b91354 10%, #BE2546 40%)':'linear-gradient(-39deg, rgba(114,4,228,0.39) 10%, rgba(96,27,191,0.79) 40%)'}}">
        <div class="shell">
            <p class="heading-1 breadcrumbs-custom-title">{{$achievment['title_'.$lang]}}</p>
            <ul class="breadcrumbs-custom-path">
                <li> class=" white" <a href="{{route('home')}}">{{trans('app.home')}}</a></li>
                <li class=" white"><a href="{{localizeURL('achievments')}}">{{trans('app.achievments')}}</a></li>
                <li class="active white">{{$achievment['title_'.$lang]}}</li>
            </ul>
        </div>
        ...
        <div class="shell shell-bigger">
            <div class="range range-ten range-50 range-sm-center range-lg-justify">
                <div class="cell-sm-7 cell-xl-6">
                    <h3>{{$achievment['title_'.$lang]}}</h3>
                    <div class="divider divider-default"></div>
                    <div class="">
                        <p class="text-spacing-sm">{!! $achievment['content_'.$lang] !!}</p>
                    </div>
                </div>
            </div>
        </div>
